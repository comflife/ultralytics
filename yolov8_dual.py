    ...
    if 'stats' in locals() and len(stats):
        ...
        if len(stats_np) == 4 and len(stats_np[3]):
            ...
            try:
                ...
                tp_count, fp_count, p, r, f1, ap, ap_class, *_ = ap_per_class(tp, conf, pred_cls, target_cls)
                print(f"[DEBUG][AP] ap_per_class output: p({p.shape}), r({r.shape}), ap({ap.shape})")
                print(f"[DEBUG][AP] p sample: {p[:10]}, min={p.min()}, max={p.max()}")
                print(f"[DEBUG][AP] r sample: {r[:10]}, min={r.min()}, max={r.max()}")
                print(f"[DEBUG][AP] ap sample: {ap[:10]}, min={ap.min()}, max={ap.max()}")
                print(f"[DEBUG][AP] f1 sample: {f1[:10]}, min={f1.min()}, max={f1.max()}")
                print(f"[DEBUG][AP] ap_class sample: {ap_class[:10]}, shape={ap_class.shape}")
