...
def main():
    ...
    if args.visualize:
        ...
        if len(loaded)==9:  # ensure full
            ...
            # --- Mixed visualizations to isolate DFL (regression) influence ---
            # 1) ONNX CLS + DEP, DUMP DFL (reg) --> shows effect if only regression differs
            mix1_outputs = []
            for i in range(3):
                # replace regression with dump; keep cls/dep from ONNX
                mix1_outputs.append(loaded[i*3])           # dump reg
                mix1_outputs.append(outputs[i*3 + 1])      # onnx cls
                mix1_outputs.append(outputs[i*3 + 2])      # onnx dep
            mix1_dets = decode(mix1_outputs, 'MIX_DUMP_DFL')
            draw(wide_bgr, mix1_dets, 'MIX1', 'mix_onxx_clsdep_dump_dfl.jpg')

            # 2) DUMP CLS + DEP, ONNX DFL (reg) --> shows effect if only dump classification/depth differ
            mix2_outputs = []
            for i in range(3):
                mix2_outputs.append(outputs[i*3])          # onnx reg
                mix2_outputs.append(loaded[i*3 + 1])       # dump cls
                mix2_outputs.append(loaded[i*3 + 2])       # dump dep
            mix2_dets = decode(mix2_outputs, 'MIX_ONNX_DFL')
            draw(wide_bgr, mix2_dets, 'MIX2', 'mix_dump_clsdep_onnx_dfl.jpg')
        else:
