...
def verify_image_label(args):
    ...
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, single_cls, with_depth = args
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                if keypoint:
                    ...
                    # Validate depth values (should be normalized 0-1)
                    depth_values = lb[:, 5]
                    assert np.all(depth_values >= 0) and np.all(depth_values <= 1), f"depth values must be normalized 0-1, got range [{depth_values.min():.3f}, {depth_values.max():.3f}]"
                else:
        ...
        return im_file, lb, shape, segments, keypoints, depth_values, nm, nf, ne, nc, msg
    except Exception as e:
        nc = 1
        msg = f"{prefix}{im_file}: ignoring corrupt image/label: {e}"
