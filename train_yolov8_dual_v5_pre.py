...
def train(cfg, opt, device, callbacks=None):
    ...
    if opt.resume:
        ...
        model = YOLO(opt.resume if isinstance(opt.resume, str) else 'last.pt')
        print(f"🔄 Resuming training from checkpoint")
    else:
        # Create new dual-stream model
        ...
        # Initialize from single-stream pretrained weights if provided
        if opt.weights and opt.weights.endswith('.pt'):
            print(f"🔄 Initializing dual-stream from single-stream pretrained: {opt.weights}")
            pretrained_ckpt = torch.load(opt.weights, map_location='cpu')
            pretrained_state = pretrained_ckpt.get('model', pretrained_ckpt)
            ...
            # Transfer weights with dual-stream adaptation
            transferred = 0
            duplicated = 0
            skipped = 0
            ...
            for name, param in pretrained_dict.items():
                if name in model_dict:
                    ...
                    # Case 1: First conv layer (3 channels -> 6 channels)
                    # Duplicate weights for RGB and Depth streams
                    if len(source_shape) == 4 and source_shape[1] == 3 and target_shape[1] == 6:
                        print(f"🔁 Duplicating {name}: {source_shape} -> {target_shape}")
                        model_dict[name][:, :3, :, :] = param  # RGB stream
                        model_dict[name][:, 3:, :, :] = param  # Depth stream (copy)
                        duplicated += 1
                    # Case 2: Exact match - direct copy
                    elif source_shape == target_shape:
                        model_dict[name] = param
                        transferred += 1
                    # Case 3: Shape mismatch - skip
                    else:
                        skipped += 1
                else:
                    skipped += 1
            ...
            print(f"✅ Transfer complete: {transferred} direct, {duplicated} duplicated, {skipped} skipped")
            print(f"🎯 Dual-stream initialized with ImageNet pretrained knowledge!")
        else:
            print("🚀 Training Dual-Stream model from scratch with random initialization")

    # Unfreeze all parameters (pretrained 모델의 경우 일부가 frozen 될 수 있음)
